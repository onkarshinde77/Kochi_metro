...
export function TrainCard({ train }: { train: Train }) {
  ...
  return (
    <Card className="flex flex-col h-full hover:shadow-lg transition-shadow min-w-[324px]">
      ...
      <CardContent className="flex-1 space-y-4 text-sm">
        ...
        <DetailRow
          ...
          value={`Expires ${new Date(
            train.fitnessCertificate.expiryDate
          ).toLocaleDateString()}`}
          valueClassName={cn(
            isCertificateExpiringSoon(train.fitnessCertificate.expiryDate) &&
              'text-destructive'
          )}
        />
      </CardContent>
